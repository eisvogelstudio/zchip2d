...
pub fn cpfclamp01(arg_f: cpFloat) callconv(.c) cpFloat {
    var f = arg_f;
    _ = &f;
    return cpfmax(@as(cpFloat, @floatCast(0.0)), cpfmin(f, @as(cpFloat, @floatCast(1.0))));
}
pub fn cpflerp(arg_f1: cpFloat, arg_f2: cpFloat, arg_t: cpFloat) callconv(.c) cpFloat {
    ...
    var t = arg_t;
    _ = &t;
    return (f1 * (@as(cpFloat, @floatCast(1.0)) - t)) + (f2 * t);
}
...
pub const cpvzero: cpVect = cpVect{
    .x = @as(cpFloat, @floatCast(0.0)),
    .y = @as(cpFloat, @floatCast(0.0)),
};
...
pub fn cpvlerp(v1: cpVect, v2: cpVect, t: cpFloat) callconv(.c) cpVect {
    ...
    _ = &v2;
    _ = &t;
    return cpvadd(cpvmult(v1, @as(cpFloat, @floatCast(1.0)) - t), cpvmult(v2, t));
}
pub fn cpvnormalize(v: cpVect) callconv(.c) cpVect {
    _ = &v;
    return cpvmult(v, @as(f64, @floatCast(1.0)) / (cpvlength(v) + 0.));
}
pub fn cpvslerp(v1: cpVect, v2: cpVect, t: cpFloat) callconv(.c) cpVect {
    ...
    _ = &dot;
    var omega: cpFloat = acos(cpfclamp(dot, @as(cpFloat, @floatCast(-1.0)), @as(cpFloat, @floatCast(1.0))));
    _ = &omega;
    if (omega < 0.001) {
        return cpvlerp(v1, v2, t);
    } else {
        var denom: cpFloat = @as(f64, @floatCast(1.0)) / sin(omega);
        _ = &denom;
        return cpvadd(cpvmult(v1, sin((@as(cpFloat, @floatCast(1.0)) - t) * omega) * denom), cpvmult(v2, sin(t * omega) * denom));
    }
...
pub fn cpBBCenter(arg_bb: cpBB) callconv(.c) cpVect {
    var bb = arg_bb;
    _ = &bb;
    return cpvlerp(cpv(bb.l, bb.b), cpv(bb.r, bb.t), @as(cpFloat, @floatCast(0.5)));
}
...
pub fn cpBBSegmentQuery(arg_bb: cpBB, arg_a: cpVect, arg_b: cpVect) callconv(.c) cpFloat {
    ...
    _ = &delta;
    var tmin: cpFloat = @as(cpFloat, @floatCast(-__builtin_inff()));
    _ = &tmin;
    var tmax: cpFloat = @as(cpFloat, @floatCast(__builtin_inff()));
    _ = &tmax;
    if (delta.x == @as(cpFloat, @floatCast(0.0))) {
        if ((a.x < bb.l) or (bb.r < a.x)) return @as(cpFloat, @floatCast(__builtin_inff()));
    } else {
    ...
    if (delta.y == @as(cpFloat, @floatCast(0.0))) {
        if ((a.y < bb.b) or (bb.t < a.y)) return @as(cpFloat, @floatCast(__builtin_inff()));
    } else {
    ...
    if (((tmin <= tmax) and (@as(cpFloat, @floatCast(0.0)) <= tmax)) and (tmin <= @as(cpFloat, @floatCast(1.0)))) {
        return cpfmax(tmin, @as(cpFloat, @floatCast(0.0)));
    } else {
        return @as(cpFloat, @floatCast(__builtin_inff()));
    }
...
pub fn cpBBIntersectsSegment(arg_bb: cpBB, arg_a: cpVect, arg_b: cpVect) callconv(.c) cpBool {
    ...
    var b = arg_b;
    _ = &b;
    return @as(cpBool, @intFromBool(cpBBSegmentQuery(bb, a, b) != @as(cpFloat, @floatCast(__builtin_inff()))));
}
...
pub fn cpBBWrapVect(bb: cpBB, v: cpVect) callconv(.c) cpVect {
    ...
    _ = &modx;
    var x: cpFloat = if (modx > @as(cpFloat, @floatCast(0.0))) modx else modx + dx;
    _ = &x;
    var dy: cpFloat = cpfabs(bb.t - bb.b);
    _ = &dy;
    var mody: cpFloat = fmod(v.y - bb.b, dy);
    _ = &mody;
    var y: cpFloat = if (mody > @as(cpFloat, @floatCast(0.0))) mody else mody + dy;
    _ = &y;
...
pub const cpTransformIdentity: cpTransform = cpTransform{
    .a = @as(cpFloat, @floatCast(1.0)),
    .b = @as(cpFloat, @floatCast(0.0)),
    .c = @as(cpFloat, @floatCast(0.0)),
    .d = @as(cpFloat, @floatCast(1.0)),
    .tx = @as(cpFloat, @floatCast(0.0)),
    .ty = @as(cpFloat, @floatCast(0.0)),
};
...
pub const CP_EXPORT = "";
pub const cpAssertSoft = @compileError("unable to translate C expr: expected ')' instead got '...'");
// lib/chipmunk2d/include/chipmunk/chipmunk.h:53:10
pub const cpAssertWarn = @compileError("unable to translate C expr: expected ')' instead got '...'");
// lib/chipmunk2d/include/chipmunk/chipmunk.h:54:10
pub const cpAssertHard = @compileError("unable to translate C expr: expected ')' instead got '...'");
// lib/chipmunk2d/include/chipmunk/chipmunk.h:58:9
...
pub const _GCC_WRAP_STDINT_H = "";
pub const _FLOAT_H___ = "";
...
pub const CP_ARBITER_GET_SHAPES = @compileError("unable to translate C expr: unexpected token ';'");
// lib/chipmunk2d/include/chipmunk/cpArbiter.h:70:9
pub const CP_ARBITER_GET_BODIES = @compileError("unable to translate C expr: unexpected token ';'");
// lib/chipmunk2d/include/chipmunk/cpArbiter.h:78:9
pub const CP_VERSION_MAJOR = @as(c_int, 7);
pub const CP_VERSION_MINOR = @as(c_int, 0);
pub const CP_VERSION_RELEASE = @as(c_int, 3);
pub const CP_CONVEX_HULL = @compileError("unable to translate C expr: unexpected token '='");
// lib/chipmunk2d/include/chipmunk/chipmunk.h:177:9
